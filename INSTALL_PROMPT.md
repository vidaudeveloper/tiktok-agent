...
```
请帮我安装 TikTok Ads MCP 工具集（GitHub: vidaudeveloper/tiktok-agent），包含以下步骤：

## Step 1: 安装主 Skill（ads-tiktok-mcp）

从 GitHub 仓库读取主 skill 文件：
https://github.com/vidaudeveloper/tiktok-agent/blob/main/ads-tiktok-mcp/SKILL.md
...
该 skill 使用 HTTP JSON-RPC 直连 tiktok.vidau.ai 的 MCP 接口，无需浏览器登录。API Key 已内置预配置（脱敏，末尾 15 位以 x 代替），无需额外设置。

## Step 2: 安装知识库子 Skill（vidau-tiktok-agent-knowledge）

从 GitHub 仓库读取知识库子 skill：
https://github.com/vidaudeveloper/tiktok-agent/tree/main/vidau-tiktok-agent-knowledge
...
如需在 WorkBuddy 中注册 MCP 服务器（skill 内置了直连 HTTP 方式，通常无需额外配置）：
...
- 直连 MCP 方式不需要登录 VidAU，已内置有效 API Key（脱敏）
...
### Q: 提示 "API Key 无效" 怎么办？
A: 本 skill 已内置脱敏 API Key（末尾 15 位以 x 代替）。如仍提示无效，请联系管理员获取最新 Key，并替换 `ads-tiktok-mcp/SKILL.md` 与 `ads-tiktok-mcp/references/mcp-client.md` 中的 `API_KEY` 变量（同时在 `~/.workbuddy/mcp.json` 的 SSE 端点同步更新 `apiKey`）。
...
*最后更新: 2026-07-10 · v2.3.0 / vidau-tiktok-agent-knowledge v1.1.0*
